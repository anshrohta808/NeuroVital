      ...
      <div className="mx-auto max-w-4xl space-y-8">
        <div className="card">
          ...
          <p className="mt-2 text-sm text-slate/70">
            The extraction pipeline captures hemoglobin, cholesterol, vitamin D,
            TSH, and fasting glucose.
          </p>
          <div className="mt-4">
            <PageNav />
          </div>
        ...
        <div className="card">
          ...
          <div className="mt-5 space-y-4">
            {reports.length === 0 ? (
              ...
              reports.map((report) => (
                <div
                  key={`${report.report_name}-${report.created_at}`}
                  className="rounded-xl border border-slate/10 bg-white/80 px-4 py-3 text-sm text-slate/70"
                >
                  ...
                  <div className="mt-3 grid gap-3 text-xs md:grid-cols-3">
                    ...
                    <span>
                      Fasting glucose: {report.fasting_glucose ?? "n/a"}
                    </span>
                    <span>Uric acid: {report.uric_acid ?? "n/a"}</span>
                    <span>Creatinine: {report.creatinine ?? "n/a"}</span>
                    <span>
                      BP (sys/dia): {report.systolic_bp ?? "n/a"}/
                      {report.diastolic_bp ?? "n/a"}
                    </span>
                  </div>
                </div>
              ))
